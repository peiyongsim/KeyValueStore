...
void tpcleader_handle_get(tpcleader_t *leader, kvrequest_t *req, kvresponse_t *res) {
  ...
  follower_t *p;
  int sockfd;
  p = tpcleader_get_primary(leader, req->key);
  sockfd = connect_to(p->host, p->port, TIMEOUT);
...
void tpcleader_handle_tpc(tpcleader_t *leader, kvrequest_t *req, kvresponse_t *res) {
  ...
  bool ret;
  // primary
  follower_t *followers[leader->redundancy];
  p = tpcleader_get_primary(leader, req->key);
  followers[0] = p;
  for (int i = 1; i < leader->redundancy; i++) {
    s = tpcleader_get_successor(leader, p);
    followers[i] = s;
    p = s;
  }
  for (int i = 0; i < leader->redundancy; i++) {
    follower_t *f = followers[i];
    sockfd = connect_to(f->host, f->port, TIMEOUT);
    kvrequest_send(req, sockfd);
    ret = kvresponse_receive(res, sockfd);
    if (!ret || strcmp(res->body, MSG_COMMIT) != 0 || res->type == ERROR) {
      commit = false;
    }
    close(sockfd);
  }
  ...
  // to all followers until succeed
  for (int j = 0; j < leader->redundancy; j++) {
      while (true) {
        follower_t *f = followers[j];
        sockfd = connect_to(f->host, f->port, TIMEOUT);
        kvrequest_send(req, sockfd);
        kvresponse_receive(res, sockfd);
        if (res->type == ACK) {
